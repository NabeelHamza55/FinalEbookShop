<div class=" p-5">
    <div class="secondpage" id="categories">    
        <div class=" py-5 font-adj">
            Featured Categories
        </div>
                <div class="row  justify-content-center align-items-center">

                    <div class="col-md-3 col-sm-12 p-3">
                        <div class="font-adjII bg-primary px-5 py-4">
                            <div class="icon-adj">
                                <i class="fas fa-images text-dark"></i>
                            </div>
                            <div>
                                <h6 class="s_tyle">Atrs & Photography</h6>
                            </div>
                                <button class="btn btn-lg">Shop now</button>  

                        </div>
                    </div>


                    <div class="col-md-3 col-sm-12 p-3">
                        <div class="font-adjII bg-secondary px-5 py-4">
                            <div class="icon-adj">
                                <i class="fas fa-utensils text-warning"></i>
                            </div>
                            <div>
                                <h6 class="s_tyle">Food |&| Drinks</h6>
                            </div>       
                                <button class="btn btn-lg">Shop now</button>  
                        </div>
                    </div>
                    

                    <div class="col-md-3 col-sm-12 p-3">
                        <div class="font-adjII bg-success px-5 py-4">
                            <div class="icon-adj">
                                <i class="fas fa-heartbeat text-danger"></i>
                            </div>
                            <div>
                                <h6 class="s_tyle">Romance & Health</h6>
                            </div>  
                            <button class="btn btn-lg">Shop now</button>     
                        </div>
                    </div>


                    <div class="col-md-3 col-sm-12 p-3">
                        <div class="font-adjII bg-warning px-5 py-4">
                            <div class="icon-adj">
                                <i class="fas fa-stethoscope text-info"></i>
                            </div>
                            <div>
                                <h6 class="s_tyle">General Knowledge</h6>
                            </div>       
                                <button class="btn btn-lg">Shop now</button>  
                        </div>
                    </div>


                </div>

        
    </div>
</div>